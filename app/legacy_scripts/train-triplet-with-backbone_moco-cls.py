...
def calculate_matrix(dict_, mode='linear'):
    ecs = list(dict_.keys())
...
def train(model, args, epoch, train_loader, static_embed_loader,
          optimizer, device, dtype, criterion, esm_model, esm_optimizer, seq_dict, batch_converter, alphabet, attentions, attentions_optimizer=None, learnable_k=None, ec_number_classifier=None, ec_classifier_optimizer=None, score_matrix=None) :
    model.train()
    total_loss = 0.
...
def main():
    ...
    score_matrix = calculate_matrix(ec_id_dict)
