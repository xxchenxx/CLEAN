    ...
    for batch, data in enumerate(train_loader):
        ...
        for i, tokens_len in enumerate(batch_lens):
            if query is None:
                ...
                anchor.append(prob.transpose(0, 1) @ token_representations[i, 1 : tokens_len - 1].squeeze(0))
        ...
        for i, tokens_len in enumerate(batch_lens):
            if query is None:
                ...
                k = key(mean) # 1 x 64
                prob = torch.softmax(q @ k.transpose(0, 1) / np.sqrt(64), 0) # N x 1
                positive.append(prob.transpose(0, 1) @ token_representations[i, 1 : tokens_len - 1].squeeze(0))
        ...
        for i, tokens_len in enumerate(batch_lens):
            if query is None:
                ...
                k = key(mean) # 1 x 64
                prob = torch.softmax(q @ k.transpose(0, 1) / np.sqrt(64), 0) # N x 1
                negative.append(prob.transpose(0, 1) @ token_representations[i, 1 : tokens_len - 1].squeeze(0))
...
def main():
    ...
    # balance = generate_balance(num_devices, len(esm_model))
    # esm_model = partition_model(esm_model, balance)
    # esm_model = Pipe(
    #     esm_model, chunks=4, checkpoint='except_last'
    # )
    batch_converter = alphabet.get_batch_converter()
    esm_model.eval()
    # use the original esm embedding distance map
    # update every epoch
    # device = get_last_device(esm_model)
    esm_emb = pickle.load(
